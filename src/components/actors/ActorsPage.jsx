        ...
        <>
            <div className="flex flex-wrap items-start relative">
                {actorsList.map(
                    (actor) => {
                        return (
                            <div key={actor.id} className="flex flex-wrap p-3 xs:p-5 w-1/2 md:w-1/3 lg:w-1/5 items-center">
                                <Link to={`/actors/${actor.id}`} className="">
                                    ...
                                    <h3 className="text-base text-white mb-2 hover:underline text-clip overflow-hidden leading-tight">{actor.name}</h3>
                                    <p className="text-amber-600 left-0">Rating:&nbsp;
                                        <span>
